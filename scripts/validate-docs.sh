...
require_contains "$readme" 'scripts/test-public-readiness\.sh' 'README includes public readiness command'
require_contains "$readme" 'validate-install\.sh` is a local smoke test because GitHub-hosted runners do not provide the Codex CLI' 'README explains install smoke CI boundary'
